...
class PlackettLuceModel:
    ...
    def _validate_hyperedges(self, hyperedges: List[Tuple]) -> None:
        ...
        for i, item in enumerate(hyperedges):
            if not isinstance(item, tuple) or len(item) != 2:
                raise ValueError(f"Each hyperedge must be (ranking, weight) tuple, got {item} at index {i}")
            
            ranking, weight = item
            if not isinstance(ranking, (tuple, list)) or len(ranking) < 2:
                raise ValueError(f"Ranking must be tuple/list with at least 2 elements at index {i}")
    ...
    def log_likelihood(self, hyperedges: List[Tuple]) -> float:
        ...
        if self.model_type == 'full':
            return compute_log_likelihood_pl(self.scores, edges, weights, edge_lengths, edge_starts)
        else:
            return compute_log_likelihood_position1(self.scores, edges, weights, edge_lengths, edge_starts)
