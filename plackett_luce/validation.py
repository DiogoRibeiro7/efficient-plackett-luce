...
def cross_validate(
    ...
    """
    Perform k-fold cross-validation on Plackett-Luce model.
    ...
    Returns:
    --------
    dict : Cross-validation results with mean and std of log-likelihoods
    """
    ...
    for fold, (train_idx, test_idx) in enumerate(kf.split(hyperedges_array)):
        ...
        if verbose:
            print(f"  PL: {ll_pl:.4f}, Projected: {ll_proj:.4f}")
    ...
    return {
        ...
        "winner": "PL"
        if np.mean(scores_pl) > np.mean(scores_projected)
        else "Projected",
    }
